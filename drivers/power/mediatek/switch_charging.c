...
static void set_jeita_charging_current(void)
{
	...
	/*0c-5c limit current to 500ma,5c-10c limit current to 850ma,45-52 limit current to 850ma */
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
		g_temp_CC_value = batt_cust_data.jeita_neg_10_to_pos_0_full_current;
		g_temp_input_CC_value = batt_cust_data.jeita_neg_10_to_pos_0_full_current;;
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set jeita_neg_10_to_pos_0_full_current charging current : %d\r\n",
			    g_temp_CC_value);
	}else if (g_temp_status == TEMP_POS_0_TO_POS_10){
		{
			if(BMT_status.temperature <= TEMP_POS_5_THRESHOLD)
			{
				g_temp_CC_value = batt_cust_data.jeita_temp_pos_0_to_pos_5_full_current;
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set pos_0_to_pos_5 charging current : %d\r\n",
					    g_temp_CC_value);
			}
			else
			{
				g_temp_CC_value = batt_cust_data.jeita_temp_pos_5_to_pos_10_full_current;
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set pos_5_to_pos_10 charging current : %d\r\n",
					    g_temp_CC_value);
			}
		}
	}else if (g_temp_status == TEMP_POS_45_TO_POS_60){
		g_temp_CC_value = batt_cust_data.jeita_temp_pos_45_to_pos_60_full_current;
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set pos_45_to_pos_60 charging current : %d\r\n",
			    g_temp_CC_value);
