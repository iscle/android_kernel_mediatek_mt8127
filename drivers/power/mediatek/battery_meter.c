...
#ifndef CV_CURRENT
/*< DTS2017011110644 rentianzhi WX430340 20170118 begin */
#define CV_CURRENT 4000 /* 400mA */
/* DTS2017011110644 rentianzhi WX430340 20170118 end >*/
/*< DTS2017011208370 zhixueyin WX422270 20170221 begin */
#define CV_CURRENT_DESAY    3000  /*300mA*/
/* DTS2017011208370 zhixueyin WX422270 20170221 end >*/
#endif

/*< DTS2017011110644  rentianzhi WX430340 20170120 begin */
static unsigned int g_step_of_qmax = 60;

#ifndef STEP_OF_QMAX
#define STEP_OF_QMAX 60
#endif
#define STEP_OF_QMAX_SCUD           50
#define STEP_OF_QMAX_SUNWODA        30
#define STEP_OF_QMAX_DESAY          30
#define SYSTEM_OFF_VOLTAGE_SCUD     3550
#define SYSTEM_OFF_VOLTAGE_SUNWODA  3500
#define SYSTEM_OFF_VOLTAGE_DESAY    3550
static signed int g_currentfactor = 100;
static kal_bool g_USE_UI_SOC = KAL_TRUE;
static unsigned int g_system_off_voltage = 3400;
/* DTS2017011110644 rentianzhi WX430340 20170120 end >*/
...
/*< DTS2017011204287  rentianzhi WX430340 20170112 begin */
/*< DTS2017011911589  rentianzhi WX430340 20170222 begin */
#define BATTERY_TYPE_SCUD 105882		//SCUD
#define BATTERY_TYPE_SUNWODA 535135		//SUNWODA
#define BATTERY_TYPE_DESAY 776147		//DESAY

#define LOWER_LEVEL 52941
#define SCUD_ID 320508
#define SUNWODA_ID 655641
#define UPPER_LEVEL 850000

#define SAMPLING_NUM 16
#define SCALE 5
/* DTS2017011911589  rentianzhi WX430340 20170222 end >*/
/* DTS2017011204287  rentianzhi WX430340 20170112 end >*/

signed int g_battery_id_voltage[] = {SCUD_ID, SUNWODA_ID, UPPER_LEVEL};

#define TOTAL_BATTERY_NUMBER 3
/*< DTS2017011208370 zhixueyin WX422270 20170221 begin */
#define POS_50_SCUD 2912
#define POS_50_SUNWODA 2878
#define POS_50_DESAY 2803

#define POS_25_SCUD 2946
#define POS_25_SUNWODA 2942
#define POS_25_DESAY 2935

#define POS_0_SCUD 3000
#define POS_0_SUNWODA 2966
#define POS_0_DESAY 2902

#define NEG_10_SCUD 2967
#define NEG_10_SUNWODA 2883
#define NEG_10_DESAY 2914

#define POS_50_H_CURRENT_SCUD 2884
#define POS_50_H_CURRENT_SUNWODA 2842
#define POS_50_H_CURRENT_DESAY 2783

#define POS_25_H_CURRENT_SCUD 2908
#define POS_25_H_CURRENT_SUNWODA 2893
#define POS_25_H_CURRENT_DESAY 2905

#define POS_0_H_CURRENT_SCUD 2413
#define POS_0_H_CURRENT_SUNWODA 2270
#define POS_0_H_CURRENT_DESAY 2296

#define NEG_10_H_CURRENT_SCUD 1144
#define NEG_10_H_CURRENT_SUNWODA 1561
#define NEG_10_H_CURRENT_DESAY 1643
/* DTS2017011208370 zhixueyin WX422270 20170221 end >*/

/* Qmax for battery */
signed int g_Q_MAX_POS_50[] = {POS_50_SCUD, POS_50_SUNWODA, POS_50_DESAY};
signed int g_Q_MAX_POS_25[] = {POS_25_SCUD, POS_25_SUNWODA, POS_25_DESAY};
signed int g_Q_MAX_POS_0[] = {POS_0_SCUD, POS_0_SUNWODA, POS_0_DESAY};
signed int g_Q_MAX_NEG_10[] = {NEG_10_SCUD, NEG_10_SUNWODA, NEG_10_DESAY};

signed int g_Q_MAX_POS_50_H_CURRENT[] = {POS_50_H_CURRENT_SCUD, POS_50_H_CURRENT_SUNWODA, POS_50_H_CURRENT_DESAY};
signed int g_Q_MAX_POS_25_H_CURRENT[] = {POS_25_H_CURRENT_SCUD, POS_25_H_CURRENT_SUNWODA, POS_25_H_CURRENT_DESAY};
signed int g_Q_MAX_POS_0_H_CURRENT[] = {POS_0_H_CURRENT_SCUD, POS_0_H_CURRENT_SUNWODA, POS_0_H_CURRENT_DESAY};
signed int g_Q_MAX_NEG_10_H_CURRENT[] = {NEG_10_H_CURRENT_SCUD, NEG_10_H_CURRENT_SUNWODA, NEG_10_H_CURRENT_DESAY};

unsigned int battery_type_id = 0;
...
void fgauge_get_profile_id(void)
{
	...
	int ret = 0;
	/*< DTS2017011911589  rentianzhi WX430340 20170222 begin */
	signed int sampling_id_vbat[SAMPLING_NUM] = {0};
	int avg_id_volt = 0;
	int i = 0;
	int j = 0;
	int temp = 0;
	...
	for (i= 0; i < SAMPLING_NUM; i++) {
		...
		if (ret != 0) {
			battery_log(BM_LOG_CRTI, "[fgauge_get_profile_id]id_volt read fail\n");
		} else {
			battery_log(BM_LOG_CRTI, "[fgauge_get_profile_id]id_volt = %d\n", id_volt);
		}
		sampling_id_vbat[i] = id_volt;
		id_volt = 0;
	}

	for (i = 0; i < SAMPLING_NUM; i++) {
		for (j = i; j < SAMPLING_NUM; j++) {
			if (sampling_id_vbat[i] > sampling_id_vbat[j]) {
				temp = sampling_id_vbat[i];
				sampling_id_vbat[i] = sampling_id_vbat[j];
				sampling_id_vbat[j] = temp;
			}
		}
	}

	for (i = SAMPLING_NUM / SCALE; i < SAMPLING_NUM - (SAMPLING_NUM / SCALE); i++) {
		num += sampling_id_vbat[i];
		n++;
		battery_log(BAT_LOG_CRTI, "sampling_id_vbat[i] = %d n = %d\n", sampling_id_vbat[i], n);
	}

	avg_id_volt = num / n;
	battery_log(BAT_LOG_CRTI, "avg_id_volt = %d\n", avg_id_volt);

	if ((sizeof(g_battery_id_voltage) / sizeof(signed int)) != TOTAL_BATTERY_NUMBER) {
		battery_log(BM_LOG_CRTI, "[fgauge_get_profile_id]error! voltage range incorrect!\n");
		return;
	}

	if ((avg_id_volt < LOWER_LEVEL) || (avg_id_volt > UPPER_LEVEL)) {
		battery_type_id = 0;
		g_fg_battery_id = 0;
		V_ID = avg_id_volt;
	} else {
		for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
			if (avg_id_volt <= g_battery_id_voltage[id]) {
				V_ID = avg_id_volt;
				g_fg_battery_id = id;
				battery_type_id = id + 1;
				break;
			}
		}
	}
	battery_log(BAT_LOG_CRTI,
		"[g_fg_battery_id] is (%d)[battery_type_id] is (%d) [avg_id_volt] is (%d)\n",
		g_fg_battery_id, battery_type_id, V_ID);
	/* DTS2017011911589  rentianzhi WX430340 20170222 end >*/
	battery_log(BM_LOG_CRTI, "[fgauge_get_profile_id]Battery id (%d)\n", g_fg_battery_id);
}
/*DTS2016122104616 liuguangchao/lwx422269  20161229 end >*/
...
void fgauge_construct_battery_profile(signed int temperature, BATTERY_PROFILE_STRUCT_P temp_profile_p)
{
	...
	for (i = 0; i < saddles; i++) {
		battery_log(BAT_LOG_CRTI, "<DOD,Voltage> at %d = <%d,%d>\r\n",
...
void fgauge_construct_r_table_profile(signed int temperature, R_PROFILE_STRUCT_P temp_profile_p)
{
	...
	for (i = 0; i < saddles; i++) {
		battery_log(BAT_LOG_CRTI, "<Rbat,VBAT> at %d = <%d,%d>\r\n",
...
#if defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
/*< DTS2017011110644 rentianzhi WX430340 20170118 begin */
static void battery_capacity_ocv2cv_transform_init(void)
{
    switch (g_fg_battery_id)
    {
        case 0: /* SCUD */
            g_step_of_qmax = STEP_OF_QMAX_SCUD;
            g_system_off_voltage = SYSTEM_OFF_VOLTAGE_SCUD;
            break;
        case 1: /* SUNWODA */
            g_step_of_qmax = STEP_OF_QMAX_SUNWODA;
            g_system_off_voltage = SYSTEM_OFF_VOLTAGE_SUNWODA;
            break;
        case 2: /* DESAY */
            g_step_of_qmax = STEP_OF_QMAX_DESAY;
            g_system_off_voltage = SYSTEM_OFF_VOLTAGE_DESAY;
            break;
        default:
            break;
    }
...
void fgauge_get_current_factor(void)
{
	...
	if (init_current == KAL_TRUE) {
		...
		if (BAT_ID_OF_DESAY == g_fg_battery_id)
		{
			current_sum = CV_CURRENT_DESAY * TEMP_AVERAGE_SIZE;
		}
		else
		{
	...
	if (BAT_ID_OF_DESAY == g_fg_battery_id)
	{
		g_currentfactor = avg_current * 100 / CV_CURRENT_DESAY;	/* calculate factor by 300ma */
	}
	else
	{
		g_currentfactor = avg_current * 100 / CV_CURRENT;	/* calculate factor by 400ma */
	}
	/* DTS2017011208370 zhixueyin WX422270 20170221 end >*/
...
signed int fgauge_get_Q_max_high_current_by_current(signed int i_current, signed short val_temp)
{
    ...
    /*< DTS2017011110644 rentianzhi WX430340 20170118 begin */
	signed int threshold = g_system_off_voltage;
    /* DTS2017011110644 rentianzhi WX430340 20170118 end >*/
	...
	for (iIndex = 0; iIndex < saddles - 1; iIndex++) {
		...
		if (OCV_temp - V_drop < threshold) {
			if (iIndex <= 1)
				ret_Q_max = g_step_of_qmax;
			else
				ret_Q_max = (iIndex - 1) * g_step_of_qmax;
			break;
		}
        /* DTS2017011110644 rentianzhi WX430340 20170118 end >*/
	}
...
if (((g_rtc_fg_soc != 0)
	...
	sw_oam_init_v2();
#endif
      /*< DTS2017020904854 zhixueyin WX422270 20170302 begin */
	if ((g_rtc_fg_soc == 0) &&
		(gFG_capacity_by_v <= FG_CAPACITY_THREHOLD)){
		g_rtc_fg_soc = SW_SHUTDOWN_CAPACITY;
		gFG_capacity_by_v = g_rtc_fg_soc;
		bm_print(BM_LOG_CRTI, "[FGADC] g_rtc_fg_soc is zero, use lowest capacity as UI SOC!\n");
	}
	/* DTS2017020904854 zhixueyin WX422270 20170302 end >*/
...
void oam_run(void)
{
	...
	BMT_status.Current_now = oam_i_2;
...
signed int battery_meter_trans_battery_percentage(kal_bool d_val)
{
	...
	/* discharging and current > 400ma */
	/*< DTS2017011208370 zhixueyin WX422270 20170221 begin */
	if (BAT_ID_OF_DESAY == g_fg_battery_id)
	{
		i_avg_current = g_currentfactor * CV_CURRENT_DESAY / 100;
	}
	else
	{
...
/*DTS2016122104616 liuguangchao/lwx422269  20161229 end >*/
    ...
    /*< DTS2017011110644 rentianzhi WX430340 20170118 begin */
    battery_capacity_ocv2cv_transform_init();
    /* DTS2017011110644 rentianzhi WX430340 20170118 end >*/
#endif
