...
/*<DTS2017012505235 rentianzhi 20170306 begin*/
#define HW_WAIT_U_TIME 10
#define HW_MAX_WAIT_U_TIME 20000
#define HW_HALF 2
/*DTS2017012505235 rentianzhi 20170306 end>*/
...
extern struct dsm_client *fs_ext4_dclient;
#endif
/*DTS2016120701717 guoyuanyuan/gwx422270 20161205 end >*/
...
/*DTS2016120701717 guoyuanyuan/gwx422270 20161205 begin >*/
#ifdef CONFIG_HUAWEI_EMMC_DSM
	int dsm_data;
	sscanf(buf, "%d", &dsm_data);
	if(dsm_data == 21002)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_ERASE_ERR,"dsm_test: DSM_EMMC_ERASE_ERR\n");
	return count;
	}
	else if(dsm_data == 21004)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_SEND_CXD_ERR,"dsm_test: DSM_EMMC_SEND_CXD_ERR\n");
	return count;
	}
	else if(dsm_data == 21005)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_READ_ERR,"dsm_test: DSM_EMMC_READ_ERR\n");
	return count;
	}
	else if(dsm_data == 21006)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_WRITE_ERR,"dsm_test: DSM_EMMC_WRITE_ERR\n");
	return count;
	}
	else if(dsm_data == 21009)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_TUNING_ERROR,"dsm_test: DSM_EMMC_TUNING_ERROR\n");
	return count;
	}
	else if(dsm_data == 21010)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_LIFE_TIME_EST_ERR,"dsm_test: DSM_EMMC_LIFE_TIME_EST_ERR\n");
	return count;
	}
	else if(dsm_data == 21011)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_PACKED_FAILURE,"dsm_test: DSM_EMMC_PACKED_FAILURE\n");
	return count;
	}
	else if(dsm_data == 21012)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_RSP_ERR,"dsm_test: DSM_EMMC_RSP_ERR\n");
	return count;
	}
	else if(dsm_data == 21013)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_HOST_TIMEOUT_ERR,"dsm_test: DSM_EMMC_HOST_TIMEOUT_ERR\n");
	return count;
	}
	else if(dsm_data == 21016)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_DYNCAP_NEEDED,"dsm_test: DSM_EMMC_DYNCAP_NEEDED\n");
	return count;
	}
	else if(dsm_data == 21017)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_SYSPOOL_EXHAUSTED,"dsm_test: DSM_EMMC_SYSPOOL_EXHAUSTED\n");
	return count;
	}
	else if(dsm_data == 21019)
	{
	DSM_EMMC_LOG(dsm_card, DSM_EMMC_CACHE_ERR,"dsm_test: DSM_EMMC_CACHE_ERR\n");
	return count;
	}
#endif
#ifdef CONFIG_HUAWEI_SDCARD_DSM
	...
	char *log_buff;
	int dsm_sdcard_data;
	sscanf(buf, "%d", &dsm_sdcard_data);
	if(dsm_sdcard_data == 20701)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD2_RESP_ERR,0);
	...
	if(dsm_sdcard_data == 20702)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD3_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20703)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD7_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20704)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD8_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20705)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD9_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20706)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_CMD55_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20707)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_ACMD41_RESP_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20708)
	{
	log_buff = dsm_sdcard_get_log(DSM_SDCARD_NO_UEVENT_REPORT,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	...
	if(dsm_sdcard_data == 20728)
	{
	log_buff = dsm_sdcard_get_log(DMS_SDCARD_HARDWARE_TIMEOUT_ERR,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	dsm_client_notify(sdcard_dclient, DMS_SDCARD_HARDWARE_TIMEOUT_ERR);
	return count;

	}
	if(dsm_sdcard_data == 20732)
	{
	log_buff = dsm_sdcard_get_log(DMS_SDCARD_MMC_BLK_ABORT,0);
	buff_len = strlen(log_buff);
	dsm_client_copy(sdcard_dclient,log_buff,buff_len + 1);
	dsm_client_notify(sdcard_dclient, DMS_SDCARD_MMC_BLK_ABORT);
	return count;
...
#ifdef CONFIG_HUAWEI_DSM
	int fs_data;
	if(dsm_sdcard_data == 21401)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR");
		dsm_client_notify(fs_ext4_dclient, DSM_FS_EXT4_ERROR);
		return count;
	}
	if(dsm_sdcard_data == 21402)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR_INODE");
		dsm_client_notify(fs_ext4_dclient, DSM_FS_EXT4_ERROR_INODE);
		return count;
	}
	if(dsm_sdcard_data == 21403)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR_FILE");
		dsm_client_notify(fs_ext4_dclient, DSM_FS_EXT4_ERROR_FILE);
		return count;
	}
	if(dsm_sdcard_data == 21404)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR_READ_SUPER");
		dsm_client_notify(fs_ext4_dclient, DSM_FS_EXT4_ERROR_READ_SUPER);
		return count;
	}
	if(dsm_sdcard_data == 21405)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR_READ_SUPER_SECOND");
		dsm_client_notify(fs_ext4_dclient, DSM_FS_EXT4_ERROR_READ_SUPER_SECOND);
		return count;
	}
	if(dsm_sdcard_data == 21406)
	{
		dsm_client_record(fs_ext4_dclient,"dsm_test: DSM_FS_EXT4_ERROR_WRITE_SUPER");
		...
		bool hw_busy_detect, struct request *req, int *gen_err)
{
	...
	u32 status;
	/*<DTS2017012505235 rentianzhi 20170306 begin*/
	u32 wait_usec = HW_WAIT_U_TIME;
	u32 max_wait_usec = HW_MAX_WAIT_U_TIME;
	/*DTS2017012505235 rentianzhi 20170306 end>*/
	...
	do {
		...
		/*<DTS2017012505235 rentianzhi 20170306 begin*/
		/*
		 * some card will keep long time busy state, flooding CMD13
		 * response ready interrupt will block other threads which
		 * need irq to wake up, so add usleep() to make other threads
		 * have a chance to run.
		 */
		usleep_range(wait_usec, wait_usec + 1);
		if (wait_usec <= (max_wait_usec / HW_HALF))
			wait_usec <<= 1;
		/*DTS2017012505235 rentianzhi 20170306 end>*/
		/*
...
#if  defined(CONFIG_ARCH_MT8127) && defined(CONFIG_MTK_EMMC_SUPPORT) && !defined(CONFIG_MTK_GPT_SCHEME_SUPPORT)
...
/*<DTS2017033010904 by liumaoxin/lwx422269 20170406 begin*/
#define CID_MANFID_SANDISK_NEW  0x45
/*DTS2017033010904 by liumaoxin/lwx422269 20170406 end> */
