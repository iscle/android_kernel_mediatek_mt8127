...
/* CMD_ACCESS_REG rCmdAccessReg; */
			...
			if (u2SubId == 0x0123) {
				...
				wlanSetPreferBandByNetwork(prAdapter, BAND_NULL, NETWORK_TYPE_AIS_INDEX);
			} else if (u2SubId == 0x1251) {
				DBGLOG(OID, TRACE, "LTE_COEX: SW SET 2.4G BAND\n");
				wlanSetPreferBandByNetwork(prAdapter, BAND_2G4, NETWORK_TYPE_AIS_INDEX);
			} else if (u2SubId == 0x1252) {
				DBGLOG(OID, TRACE, "LTE_COEX: SW SET 5G BAND\n");
				wlanSetPreferBandByNetwork(prAdapter, BAND_5G, NETWORK_TYPE_AIS_INDEX);
			}
#endif
		}
		break;
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	/*DTS2016122810981 liuguangchao WX422269 20170116 begin*/
	if(u4SetBufferLen > sizeof(prAdapter->prGlueInfo->aucWapiAssocInfoIEs)){
		DBGLOG(SEC, INFO, "u4SetBufferLen overflow,size is %u > 42!\n", u4SetBufferLen);
		return WLAN_STATUS_INVALID_LENGTH;
	}
	/*DTS2016122810981 liuguangchao WX422269 20170116 end*/
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	if(u4SetBufferLen > sizeof(prAdapter->prGlueInfo->aucWSCAssocInfoIE)){
		DBGLOG(SEC, INFO, "u4SetBufferLen length overflow, size is %u \n", u4SetBufferLen);
		return WLAN_STATUS_INVALID_LENGTH;
	}
	/*DTS2016122810981 liuguangchao WX422269 20170116 end*/
