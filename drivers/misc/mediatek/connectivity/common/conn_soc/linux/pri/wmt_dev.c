...
static ssize_t wmt_dev_proc_for_aee_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	INT32 retval = 0;
	UINT32 len = 0;
	/*< DTS2017031304135 liuguangchao lwx422269 20170315 begin */
	return 0;
	/* DTS2017031304135 liuguangchao lwx422269 20170315 end >*/
	/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
	WMT_LOUD_FUNC("%s: count %d pos %lld\n", __func__, count, *f_pos);
	/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
	if (0 == *f_pos) {
		pBuf = wmt_lib_get_cpupcr_xml_format(&len);
		g_buf_len = len;
		/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
		WMT_LOUD_FUNC("wmt_dev:wmt for aee buffer len(%d)\n", g_buf_len);
		/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
	}
	...
	if (g_buf_len >= count) {
		...
		if (retval) {
		    /* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
			WMT_LOUD_FUNC("copy to aee buffer failed, ret:%d\n", retval);
			/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
			retval = -EFAULT;
			goto err_exit;
		}
		...
		*f_pos += count;
		...
		pBuf += count;
		/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
		WMT_LOUD_FUNC("wmt_dev:after read,wmt for aee buffer len(%d)\n", g_buf_len);
		/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
		...
		*f_pos += g_buf_len;
		...
		WMT_LOUD_FUNC("wmt_dev: no data available for aee\n");
		/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
		retval = 0;
	}
err_exit:
...
INT32 wmt_dev_read_file(PUINT8 pName, const PPUINT8 ppBufPtr, INT32 offset, INT32 padSzBuf)
{
	...
	INT32 read_len;
	PVOID pBuf;
	/*< DTS2016112108169 wwx427566/wangxuan 20161130 begin*/
	loff_t pos;
	char __user *p;
	/*< DTS2016112108169 wwx427566/wangxuan 20161130 end*/
...
ssize_t WMT_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (copySize > 0) {
		...
		if (!strncasecmp(wrBuf, "ok", NAME_MAX)) {
			/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
			WMT_LOUD_FUNC("resp str ok\n");
			/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
			/* pWmtDevCtx->cmd_result = 0; */
			wmt_lib_trigger_cmd_signal(0);
		} else {
			/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
			WMT_LOUD_FUNC("warning resp str (%s)\n", wrBuf);
			/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
...
ssize_t WMT_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (test_and_clear_bit(WMT_STAT_CMD, &pWmtDevCtx->state)) {
		...
		/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
		WMT_LOUD_FUNC("copy cmd to user by read:%s\n", localBuf);
		/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
...
static int WMT_open(struct inode *inode, struct file *file)
{
	long ret;
	/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
	WMT_LOUD_FUNC("major %d minor %d (pid %d)\n", imajor(inode), iminor(inode), current->pid);
	/* DTS2017030701538  wangxuan/wwx427566 20170316  end>*/
	ret = wait_event_timeout(gWmtInitWq, gWmtInitDone != 0, msecs_to_jiffies(WMT_DEV_INIT_TO_MS));
	if (!ret) {
		/* < DTS2017030701538  wangxuan/wwx427566 20170316 begin */
		WMT_LOUD_FUNC("wait_event_timeout (%d)ms,(%d)jiffies,return -EIO\n",
