...
#define CLIENT_NAME_DEV_WIFI		"dsm_wifi"
static int rgb_wifi_debug_mask= 1;
static struct dsm_dev dsm_wifi= {
	.name 		= CLIENT_NAME_DEV_WIFI,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define WIFI_ERR(x...) do {\
    if (rgb_wifi_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define WIFI_INFO(x...) do {\
    if (rgb_wifi_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define WIFI_FLOW(x...) do {\
    if (rgb_wifi_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
struct dsm_client *wifi_dclient = NULL;
int wifi_dsm_report_err(int dsm_err_no, char *err_msg, int err_code)
{
	int size = 0;
/*DTS2016061000116 sijun  swx353748 20160610 begin */
	if (FACTORY_BOOT == get_boot_mode() )
	return 0;
/*DTS2016061000116 sijun  swx353748 20160610 end */
	if(dsm_client_ocuppy(wifi_dclient)){
		/* buffer is busy */
		WIFI_ERR("%s: buffer is busy!, errno = %d\n", __func__,dsm_err_no);
		return -EBUSY;
	}

	WIFI_INFO("%s user buffer apply successed, dsm_err_no=%d, err_code=%d!\n",__func__, dsm_err_no, err_code);

	size = dsm_client_record(wifi_dclient, "err_msg:%s;err_code:%d;\n",err_msg,err_code);
...
/*< DTS2016120105146 mengpan/mwx247577 20161201 begin */
#ifdef CONFIG_HUAWEI_DSM
	wifi_dclient = dsm_register_client(&dsm_wifi);
	if(!wifi_dclient)
		WIFI_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
/* DTS2016120105146 mengpan/mwx247577 20161201 end >*/
...
/*< DTS2016120105146 mengpan/mwx247577 20161201 begin */
	...
	if(wifi_dclient)
	{
	dsm_unregister_client(wifi_dclient,&dsm_wifi);
	wifi_dclient=NULL;
	}
#endif
/* DTS2016120105146 mengpan/mwx247577 20161201 end >*/
