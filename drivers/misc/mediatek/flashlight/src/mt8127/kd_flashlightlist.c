...
/* FIXED ME, please! */
#if 0				/* defined(DUMMY_FLASHLIGHT) */
	{dummyFlashlightInit},
#else
/* #warning "FIXED ME, please!" */
#endif
...
#define PK_DBG_FUNC(fmt, arg...)    pr_debug(KERN_INFO PFX "%s: " fmt, __func__ , ##arg)

#define PK_WARN(fmt, arg...)        pr_debug(KERN_WARNING PFX "%s: " fmt, __func__ , ##arg)
#define PK_NOTICE(fmt, arg...)      pr_debug(KERN_NOTICE PFX "%s: " fmt, __func__ , ##arg)
#define PK_INFO(fmt, arg...)        pr_debug(KERN_INFO PFX "%s: " fmt, __func__ , ##arg)
...
#define PK_ERR(fmt, arg...)         pr_err(KERN_ERR PFX "%s: " fmt, __func__ , ##arg)
#else
#define PK_DBG(a, ...)
#define PK_TRC(a, ...)
#define PK_VER(a, ...)
...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(flashlight_pinctrl)) {
		PK_ERR("Cannot find flashlight pinctrl!");
		ret = PTR_ERR(flashlight_pinctrl);
	}
	/* Flashlight HWEN pin initialization */
	flashlight_hwen_high = pinctrl_lookup_state(flashlight_pinctrl, "enable_high");
	if (IS_ERR(flashlight_hwen_high)) {
		ret = PTR_ERR(flashlight_hwen_high);
		PK_ERR("%s : pinctrl err, flashlight_hwen_high\n", __func__);
	}

	flashlight_hwen_low = pinctrl_lookup_state(flashlight_pinctrl, "enable_low");
	if (IS_ERR(flashlight_hwen_low)) {
		ret = PTR_ERR(flashlight_hwen_low);
		PK_ERR("%s : pinctrl err, flashlight_hwen_low\n", __func__);
	}
	return ret;
}
...
int default_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	int i4RetValue = 0;
...
#endif
{
	...
	case FLASH_IOC_GET_MAIN_PART_ID:
		partId = strobe_getPartId(e_CAMERA_MAIN_SENSOR, -1);	/* Todo: strobe id: -1 */
	...
	case FLASH_IOC_GET_SUB_PART_ID:
		partId = strobe_getPartId(e_CAMERA_SUB_SENSOR, -1);
	...
	case FLASH_IOC_GET_MAIN2_PART_ID:
		partId = strobe_getPartId(e_CAMERA_MAIN_2_SENSOR, -1);
