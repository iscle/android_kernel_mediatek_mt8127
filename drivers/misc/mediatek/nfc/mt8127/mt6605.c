...
#define I2C_ID_NAME "mediatek, IRQ_NFC"
			...
			const struct i2c_device_id *id)
{
	...
	named_gpio = of_get_named_gpio(node, "nfc-gpios", 0);    /* get virtual gpio number */
	...
	gpio_request(named_gpio, "nfc-pins");
...
#if !defined(CONFIG_MTK_LEGACY)
		value = gpio_get_value(named_gpio);    /* This is changed from 8127 M version */
#else
...
late_initcall(mt6605_dev_init);
module_exit(mt6605_dev_exit);

MODULE_AUTHOR("Thomas-CH Chen");
