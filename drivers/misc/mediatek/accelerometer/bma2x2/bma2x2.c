...
/* DTS2016121309782 lwx422271 20161213 begin */
#ifdef CONFIG_GET_HARDWARE_INFO
#define BMA2x2_HARDWARE_INFO "bosch,bma253"
#endif
/* DTS2016121309782 lwx422271 20161213 end */
...
/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,begin @20161214 */
#ifdef CONFIG_HUAWEI_DSM
#include <dsm/dsm_pub.h>
#include <linux/gpio.h>
#include <mt-plat/mt_boot.h>
#define BMA2X2_GPIO_HW_INFO  "i2c sda num:%d,status:%d;i2c scl num:%d,status:%d;error type:%d\n"
#define BMA2X2_GPIO_OFFSET 881
#define BMA2X2_GPIO_I2C_SDA 77
#define BMA2X2_GPIO_I2C_SCL 78
#define BMA2X2_GET_GPIO_VALUE(x)  gpio_get_value(x)
#define BMA2X2_MSG_MAX_LEN 256
#define BMA2X2_SHOW_GPIO_INFO  \
        BMA2X2_GPIO_I2C_SDA,BMA2X2_GET_GPIO_VALUE((BMA2X2_GPIO_I2C_SDA+BMA2X2_GPIO_OFFSET)),\
        BMA2X2_GPIO_I2C_SCL,BMA2X2_GET_GPIO_VALUE((BMA2X2_GPIO_I2C_SCL+BMA2X2_GPIO_OFFSET))

#define CLIENT_NAME_BMA2X2 "dsm_sensor_bma253"
struct dsm_client *bma2x2_dclient = NULL;
struct dsm_dev bma2x2_dsm_dev = {
    .name = CLIENT_NAME_BMA2X2,// dsm client name
    .fops = NULL,// options
    .buff_size = DSM_SENSOR_BUF_MAX,// buffer size
};
int bma2X2_dsm_report_err(int errno,int type)
{
    int size = 0;
    char *msg = NULL;
    ...
    if(!bma2x2_dclient)
    {
        GSE_ERR("%s: bma2x2_dclient is null\n", __func__);
        return -EFAULT;
    }

    msg=(char *)kmalloc(BMA2X2_MSG_MAX_LEN, GFP_KERNEL);
    if (!msg)
    {
        GSE_ERR("%s: kmalloc failed\n", __func__);
        kfree(msg);
        return -ENOMEM;
    }

    if(dsm_client_ocuppy(bma2x2_dclient))
    {
        /* buffer is busy */
        GSE_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
        kfree(msg);
        return -EBUSY;
    }
    memset(msg,0,BMA2X2_MSG_MAX_LEN);
    snprintf(msg,BMA2X2_MSG_MAX_LEN,BMA2X2_GPIO_HW_INFO,BMA2X2_SHOW_GPIO_INFO,type);
    size = dsm_client_record(bma2x2_dclient,msg);
    dsm_client_notify(bma2x2_dclient, errno);
    kfree(msg);
    return size;
}
#endif
/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,end @20161214 */
			...
			u8 addr, u8 *data, u8 len)
{
	...
	{
		#ifdef CONFIG_HUAWEI_DSM
		bma2X2_dsm_report_err(DSM_GS_I2C_ERROR,-EINVAL);
		#endif
		return -EINVAL;
	}
	/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,end @20161214 */
	...
	if (err != 2) {
		/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,begin @20161214 */
		#ifdef CONFIG_HUAWEI_DSM
		bma2X2_dsm_report_err(DSM_GS_DATA_ERROR,-EIO);
		#endif
		/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,end @20161214 */
...
/* DTS2016121309782 lwx422271 20161213 begin */
#ifdef CONFIG_GET_HARDWARE_INFO
	register_hardware_info(G_SENSOR,BMA2x2_HARDWARE_INFO);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	if (!bma2x2_dclient) {
		bma2x2_dclient = dsm_register_client(&bma2x2_dsm_dev);
	}
	if(bma2x2_dclient==NULL){
		printk("dsm_register_client bma2x2_dclient failed\n");
	}
#endif
/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,end @20161214 */
...
/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,begin @20161214 */
	...
	if(bma2x2_dclient)
	{
		dsm_unregister_client(bma2x2_dclient,&bma2x2_dsm_dev);
		bma2x2_dclient=NULL;
	}
#endif
/*DTS2016122101440,yuquan/ywx422261 added DSM for acc sensor:bma2x2,end @20161214 */
