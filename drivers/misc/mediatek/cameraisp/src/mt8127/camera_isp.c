...
typedef enum {
	ISP_CAM0_IRQ_IDX = 0,	/* CAM0 IRQ */
	ISP_CAM1_IRQ_IDX,		/* CAM1 IRQ */
	ISP_CAM2_IRQ_IDX,		/* CAM2 IRQ */
	ISP_CAM_IRQ_IDX_NUM
} ISP_CAM_IRQ_ENUM;
...
struct ISP_CLK_STRUCT ispclk[] = {
	...
	{"IMG_SEN_CAM", NULL},
	{"TOP_CAMTG_SEL", NULL},
	{"TOP_UNIVPLL_D26", NULL},
	{"TOP_UNIVPLL2_D2", NULL},
};

/* clk idx:4, for select MCLK clock source */
enum {
#ifndef CONFIG_MTK_SMI_VARIANT
	CLKIDX_TOP_CAMTG_SEL = 6,
#else
	CLKIDX_TOP_CAMTG_SEL = 4,
#endif
	CLKIDX_TOP_UNIVPLL_D26,
	CLKIDX_TOP_UNIVPLL2_D2,
};

/* MTCMOS/Clock check +++ */
unsigned long g_scpsys_baseaddr;
unsigned long g_ddrphy_baseaddr;
unsigned long g_emiVA;
unsigned long g_gpioVA;

/* original defined in mt_reg_base.h */
#ifndef EMI_BASE
#define EMI_BASE	(g_emiVA)
#endif
#ifndef GPIO_BASE
#define GPIO_BASE	(g_gpioVA)
#endif
...
#define CAMINF_BASE						(gISPSYS_Reg[ISP_IMGSYS_CONFIG_BASE_ADDR])
...
#define ISP_MIPI_ANA_ADDR               0x10010000
...
static MINT32 ISP_DumpReg(MVOID)
{
	...
	for (i = 0x44; i <= 0x48; i += 4)
		LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_ADDR + i), (unsigned int)ISP_RD32((void *)(ISP_ADDR + i)));
	...
	/* CDP */
	LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_ADDR + 0xCC0), (unsigned int)ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
	LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_ADDR + 0xD40), (unsigned int)ISP_RD32((void *)(ISP_ADDR + 0xD40)));
	...
	LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_ADDR_CAMINF), (unsigned int)ISP_RD32((void *)ISP_ADDR_CAMINF));
	LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_ADDR + 0x150), (unsigned int)ISP_RD32((void *)(ISP_ADDR + 0x150)));
...
Rdy should be 1    at idle or end of tile, if not 0, mdp isn't rdy
...
static int ISPDbgPortDump(int params)
{
	...
	LOG_DBG("(0x%08x 0x%08x)", (unsigned int)ISP_REG_ADDR_INT_STATUSX,
			(unsigned int)ISP_RD32((void *)ISP_REG_ADDR_INT_STATUSX));
	LOG_DBG(" <");
	LOG_DBG("DMA_INTX:>");
	LOG_DBG("(0x%08x 0x%08x)", (unsigned int)ISP_REG_ADDR_DMA_INTX,
			(unsigned int)ISP_RD32((void *)ISP_REG_ADDR_DMA_INTX));
	LOG_DBG(" <");
...
int ISPRegDump(int params, int level)
{
	MINT32 Ret = 0;
...
/* export the isp power status to dvfs */
bool isp_power_on_off(void)
{
	return (bool)g_EnableClkCnt;
}
EXPORT_SYMBOL(isp_power_on_off);
...
static MVOID ISP_EnableClock(MBOOL En)
{
	...
	if (En) {/* enable clock. */
		...
		/*LOG_INF("[MTCMOS CHECK]===> [0x1000660c]=0x%x , [0x10006610]=0x%x ",
		...
		/*
		LOG_INF("[VENCPLL CHECK]===> [0x1000F800]=0x%x , [0x1000F80c]=0x%x ",
			ISP_RD32(g_ddrphy_baseaddr + 0x800), ISP_RD32(g_ddrphy_baseaddr + 0x80c));
		LOG_INF("[IMG_CG_CON CHECK]===> [0x%lx]=0x%x", ISP_IMGSYS_BASE, ISP_RD32(ISP_IMGSYS_BASE));

		LOG_DBG("([0x%lx]:0x%08x [0x%lx]:0x%08x)",
			ISP_REG_ADDR_CTL_DBG_SET, ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
			ISP_REG_ADDR_CTL_DBG_PORT, ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
		*/
...
#define SMI_LARB_NUMBER 5
	...
	ISP_WR32(0xF0202000 + 0x214, 0xfff);
	...
	ISP_WR32(SMI_LARB3 + 0x14, 0x400420);
	...
	if (copy_from_user(&rt_buf_ctrl, (void *)Param, sizeof(ISP_BUFFER_CTRL_STRUCT)) == 0) {
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
				...
				iBuf = ((_imgo_ == rt_dma) ? imgo_fbc.Bits.RCNT : img2o_fbc.Bits.RCNT) - 1;
				...
				for (i = 0; i < g_deque_buf.count; i++) {
					...
					if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
						LOG_DBG("[rtbc][DEQ]:T\"%d.%06d\",i(%d),iBuf+i(%d),V(0x%x),P(0x%x),O(0x%x),2O(0x%x)",
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
	...
	if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
		LOG_DBG("[rtbc]dropCnt(%ld)", g_pstRTBuf->dropCnt);
...
static __tcmfunc irqreturn_t ISP_Irq(MINT32  Irq, MVOID *DeviceId)
{
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_TG1_ERR_ST) {
		...
		if (senifIntStA & 0x01) {
			...
			if (regVal & 0x01) {
				...
				LOG_DBG("4020(0x%08x),4024(0x%08x)", ISP_RD32((void *)(ISP_ADDR + 0x20)), ISP_RD32((void *)(ISP_ADDR + 0x24)));
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST) {
		...
		if ((sof_pa != 0) && (sof_pa == g_oldImg2oAddr) && g_newImg2oAddr != DEFAULT_PA &&
			...
			LOG_DBG("[rtbc][SOF]2o,new(0x%x)", g_newImg2oAddr);
...
#else
			/* Check if clck is valid */
			if ((strcmp("TOP_CAMTG_SEL", ispclk[CLKIDX_TOP_CAMTG_SEL].clkname) != 0)
			    || (ispclk[CLKIDX_TOP_CAMTG_SEL].clock == NULL)
			    || (strcmp("TOP_UNIVPLL_D26", ispclk[CLKIDX_TOP_UNIVPLL_D26].clkname) != 0)
			    || (ispclk[CLKIDX_TOP_UNIVPLL_D26].clock == NULL)
			    || (strcmp("TOP_UNIVPLL2_D2", ispclk[CLKIDX_TOP_UNIVPLL2_D2].clkname) != 0)
			    || (ispclk[CLKIDX_TOP_UNIVPLL2_D2].clock == NULL)) {
				LOG_ERR("set ISP_SENSOR_FREQ_CTRL error, clockname not match or clock is NULL");
				break;
			}

			if (senFreq == 1 /*CAM_PLL_48_GROUP */)
				Ret = clk_set_parent(ispclk[CLKIDX_TOP_CAMTG_SEL].clock,
									ispclk[CLKIDX_TOP_UNIVPLL_D26].clock);
			else if (senFreq == 2 /*CAM_PLL_52_GROUP */)
				Ret = clk_set_parent(ispclk[CLKIDX_TOP_CAMTG_SEL].clock,
									ispclk[CLKIDX_TOP_UNIVPLL2_D2].clock);
#endif
		} else {
...
static unsigned long of_get_nodeVA(const char *comp_str)
{
	...
	if (NULL == dev_node)
		...
		LOG_INF("of_find_compatible_node(%s) success !!", comp_str);

		plat_dev = of_find_device_by_node(dev_node);
		if (WARN_ON(!plat_dev))
			LOG_ERR("plat_dev NULL");

		LOG_INF("of_find_device_by_node(dev_node) success !!");

		ret_baseaddr = (unsigned long)of_iomap(plat_dev->dev.of_node, 0);
		LOG_INF("(%s)ret_baseaddr PA=0x%lx, VA = 0x%lx",
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
	...
	for (i = 0; i < ISP_CAM_BASEADDR_NUM; i++) {
		...
		/*LOG_INF("DT, i=%d, name=%s, map_addr=0x%lx\n", i, pDev->dev.of_node->name, gISPSYS_Reg[i]);*/
		LOG_INF("DT, i=%d, PA(0x%lx), map_VA=0x%lx\n", i, of_getISPPA(pDev->dev.of_node, i), gISPSYS_Reg[i]);
	}

	/* Save MTCMOS/Clock status reg addr for checking */
	g_scpsys_baseaddr = of_get_nodeVA("mediatek,mt8127-scpsys");
	g_ddrphy_baseaddr = of_get_nodeVA("mediatek,mt8127-ddrphy");
	g_emiVA = of_get_nodeVA("mediatek,mt8127-emi");
	g_gpioVA = of_get_nodeVA("mediatek,mt8127-pctl-a-syscfg");
...
#else /* Not define CONFIG_OF, No DT */
...
*
********************************************************************************/
static int ISP_DumpRegToProc(struct seq_file *m, void *v)
{
	MUINT32 i = 0;
	seq_printf(m, " MT ISP Register\n");
	seq_printf(m, "====== top ====\n");
...
static int CAMIO_DumpRegToProc(struct seq_file *m, void *v)
{
	seq_printf(m, "reg_0x%08X = 0x%X\n", (unsigned int)(ISP_ADDR_CAMINF + proc_regOfst),
...
static int ISP_DumpRegToProc_open(struct inode *inode, struct file *file)
{
    return single_open(file, ISP_DumpRegToProc, NULL);
}

static int CAMIO_DumpRegToProc_open(struct inode *inode, struct file *file)
{
...
*
********************************************************************************/
/*#define GPIO_BASE_ADDR           0xF0005000*/ /*remove redefinition */
#if 0 /* remove used function */
...
int setMCLKDrivingCurrent(unsigned long ioDrivingCurrent)
{
	...
	LOG_DBG("Write => Addr: 0x%08X, Write Data: 0x%08X. Read Data: 0x%08X.", GPIO_BASE_ADDR + Addr, Data, ioread32((void *)(GPIO_BASE_ADDR + Addr)));
...
static const struct of_device_id disp_pm_id_table[] = {
	{ .compatible = "mediatek,mt8127-isp_display",},
	{ },
};
