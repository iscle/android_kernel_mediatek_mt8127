...
unsigned int hdmi_reg_pa_base[HDMI_REG_NUM] = {
	0x14015000, 0x11013000, 0x10012000
};
...
const char *hdmi_use_module_name_spy(enum HDMI_REF_MODULE_ENUM module)
{
	switch (module) {
	case AP_CCIF0:
		return "mediatek,mt8127-apmixedsys";	/* TVD//PLL */
	case TOPCK_GEN:
		return "mediatek,mt8127-topckgen";
	case INFRA_SYS:
		return "mediatek,mt8127-infracfg";
	case MMSYS_CONFIG:
		return "mediatek,mt8127-mmsys";
	case GPIO_REG:
		return "mediatek,mt8127-pinctrl";
	case DPI1_REG:
		return "mediatek,mt8127-disp-dpi1";
/*	case EFUSE_REG:
		return "mediatek,mt8127-disp_config2"; */
	case GIC_REG:
		return "mediatek,mt8127-mcucfg";
/*	case PERISYS_REG:
...
/*	case MMSYS_POWER:
	...
	case INFRA_SYS_CEC:
		return "clk_cec_pdn";	/* cec module clock */
	case MMSYS_HDMI_PLL:
		return "clk_hdmi_pll";
...
void hdmi_enablecolorbar(void)
{
	HDMI_DRV_FUNC();
	internal_hdmi_write(hdmi_ref_reg[DPI1_REG]+0xf00, 0x41);
}
...
void hdmi_read(unsigned int u2Reg, unsigned int *p4Data)
{
	...
	case 0x14015000:
		internal_hdmi_read(hdmi_reg[HDMI_SHELL] + u2Reg - 0x14015000, p4Data);
		break;

	case 0x11013000:
		internal_hdmi_read(hdmi_reg[HDMI_DDC] + u2Reg - 0x11013000, p4Data);
		break;

	case 0x11012000:
		internal_hdmi_read(hdmi_reg[HDMI_CEC] + u2Reg - 0x11012000, p4Data);
		break;
	...
	case 0x14014000:
		internal_hdmi_read(hdmi_ref_reg[DPI1_REG] + u2Reg - 0x14014000, p4Data);
		break;
...
void hdmi_write(unsigned int u2Reg, unsigned int u4Data)
{
	...
	case 0x14015000:
		...
		vCaHDMIWriteReg(u2Reg - 0x14015000, u4Data);
#else
		internal_hdmi_write(hdmi_reg[HDMI_SHELL] + u2Reg - 0x14015000, u4Data);
#endif
		break;

	case 0x11013000:
		internal_hdmi_write(hdmi_reg[HDMI_DDC] + u2Reg - 0x11013000, u4Data);
		break;

	case 0x11012000:
		internal_hdmi_write(hdmi_reg[HDMI_CEC] + u2Reg - 0x11012000, u4Data);
		break;
	...
	case 0x14014000:
		internal_hdmi_write(hdmi_ref_reg[DPI1_REG] + u2Reg - 0x14014000, u4Data);
		break;
...
void hdmi_module_init(void)
{
	...
	vInitAvInfoVar();
/*fix me here, or the hdmi interrupt cannot be trriger */
#if 0
	if (request_irq(MT_CEC_IRQ_ID, hdmi_irq_handler, IRQF_TRIGGER_LOW, "mtkhdmi", NULL) < 0)
		RETNULL(1);
#endif
...
void hdmi_timer_impl(void)
{
	if (hdmi_hdmiinit == 0) {
		...
		if (request_irq(hdmi_irq, hdmi_irq_handler, IRQF_TRIGGER_LOW, "mtkhdmi", NULL) < 0) {
			pr_err("request_irq hdmi_irq error\n");
		}
#endif
		if ((hdmi_hotplugstate == HDMI_STATE_HOT_PLUG_OUT)
		    && (bCheckPordHotPlug(PORD_MODE | HOTPLUG_MODE) == TRUE)) {
...
{
	int i;
	static int is_clock_on = false;

	if (bEnable && is_clock_on == false) {
		...
		is_clock_on = true;
	} else if(!bEnable && is_clock_on == true){
		...
		pr_err("[hdmi_clock_enable] error,bEnable=%d, is_clock_on=%d\n", bEnable, is_clock_on);
	}
}
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8127-disp-hdmi");
#if 1
	/* Get 5V DDC Power Control Pin */
...
void hdmi_drv_audio_capablity(void)
{

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] != 0) {
			Cap_MAX_channel = HDMI_CHANNEL_2;
			pr_err("[HDMI]SUPPORT PCM Max 2CH\n");
		}

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[4] != 0) {
			Cap_MAX_channel = HDMI_CHANNEL_6;
			pr_err("[HDMI]SUPPORT PCM Max 6CH\n");
		}

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[5] != 0) {
			Cap_MAX_channel = HDMI_CHANNEL_7;
			pr_err("[HDMI]SUPPORT PCM Max 7CH\n");
		}

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[6] != 0) {
		Cap_MAX_channel = HDMI_CHANNEL_8;
		pr_err("[HDMI]SUPPORT PCM Max 8CH\n");
	}

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] & 0x01)
		Cap_SampleRate = HDMI_SAMPLERATE_32;

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] & 0x02)
		Cap_SampleRate = HDMI_SAMPLERATE_44;

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] & 0x04)
		Cap_SampleRate = HDMI_SAMPLERATE_48;

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] & 0x10)
		Cap_SampleRate = HDMI_SAMPLERATE_96;

	if (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0] & 0x40)
...
int hdmi_drv_get_external_device_capablity(void)
{
	...
	hdmi_drv_audio_capablity();

	pr_err("MAX_channel: %d, Samplebit: %d, SampleRate: %d\n", Cap_MAX_channel, Cap_Samplebit, Cap_SampleRate);
...
const struct HDMI_DRIVER *HDMI_GetDriver(void)
{
	static const struct HDMI_DRIVER HDMI_DRV = {
		...
		.getcapablity = hdmi_drv_get_external_device_capablity,
	};
