		...
		for (i = 0; i < 8; i++) {
			...
			for (k = 0; k < 16; k++) {
				if (k == 15) {
					...
					if ((j + (i * 16 + k)) < EDID_SIZE) {
						if(_bEdidData[j+ (i*16+k)]> 0x0f)
							pr_err("%2x  ",_bEdidData[j + (i * 16 + k)]);
						else
							pr_err("0%x  ",_bEdidData[j + (i * 16 + k)]);
					}
				}
...
void hdmi_AppGetEdidInfo(HDMI_EDID_T *pv_get_info)
{
	...
	pv_get_info->ui4_ntsc_resolution =
	    (_HdmiSinkAvCap.
	     ui4_sink_cea_ntsc_resolution | _HdmiSinkAvCap.ui4_sink_dtd_ntsc_resolution);
	...
	pv_get_info->ui4_hdmi_pcm_ch_type =
	    ((_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[0]) |
	     (_HdmiSinkAvCap.
	      ui1_sink_pcm_ch_sampling[4] << 8) | (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[6] <<
						   16));
	pv_get_info->ui4_hdmi_pcm_ch3ch4ch5ch7_type =
	    ((_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[1]) |
	     (_HdmiSinkAvCap.ui1_sink_pcm_ch_sampling[2] << 8) | (_HdmiSinkAvCap.
								  ui1_sink_pcm_ch_sampling[3] << 16)
