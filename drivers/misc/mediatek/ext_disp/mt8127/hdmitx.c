...
void *hdmi_pattern_allocate_secure_memory(int size)
{
	...
	/* Allocate */
	ret =
	    KREE_AllocSecurechunkmem(hdmi_pattern_secure_memory_session_handle(), &mem_handle, 4096,
				     size);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_err("KREE_AllocSecurechunkmem fail, ret=%d\n", ret);
		return NULL;
	}

	pr_info("KREE_AllocSecurchunkemem handle=0x%x\n", mem_handle);
