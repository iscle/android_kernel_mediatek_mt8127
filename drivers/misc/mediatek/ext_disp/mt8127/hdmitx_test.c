...
#define HDMI_LOG(fmt, arg...) do { \
		if (hdmi_log_on) {\
			pr_err("[hdmi]%s,#%d ", __func__, __LINE__); \
			pr_err(fmt, ##arg); } \
	} while (0)
...
struct HDMI_DRIVER *hdmi_drv = NULL;
...
typedef struct {
	...
	/* whether memory is forced to be rotated in Camera,
	no need to set orientation again */
	bool is_force_portrait;
	/* whether forced portrait mode, priority is higher than is_force_fullscreen */
	...
	/* Store orientation setting when HDMI driver is_force_fullscreen */
	int orientation_store_portrait;
	/* Store orientation setting when HDMI driver is_force_portrait */
...
static unsigned int temp_mva_r, temp_mva_w, temp_va;
static unsigned int hdmi_va, hdmi_mva_r, hdmi_mva_w;
#endif
...
static uint32_t const DPI_PAD_CON = 0xf2080900;
static uint32_t const NLI_ARB_CS = 0xf100d014;
...
static struct task_struct *hdmi_update_task;
...
void hdmi_update_impl(void)
{
	...
	if (temp_va != 0 && hdmi_va != 0) {
		...
		case 270:
			...
			{
				pddp.dstW =
				    ALIGN_TO(p->lcm_height * p->hdmi_height / p->lcm_width * 95 /
					     100, 4);
		...
		/* + hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * hdmi_bpp + dstOffset; */
...
static HDMI_STATUS hdmi_drv_init(void)
{
	...
	/* pr_err("[hdmi]%s, hdmi_width=%d, hdmi_height=%d\n", __func__, p->hdmi_width, p->hdmi_height); */
	...
	m4u_dma_cache_maint(M4U_CLNTMOD_WDMA, temp_va, tmpBufferSize, DMA_BIDIRECTIONAL);
...
int hdmi_dst_display_path_config(bool enable)
{
	...
	if (enable) {
		...
		/* + p->hdmi_width * p->hdmi_height * hdmi_bpp * hdmi_buffer_read_id; */
...
static int __init hdmi_init(void)
{
	...
	if (platform_device_register(&hdmi_device))
		pr_err("[hdmi]failed to register hdmi device");
