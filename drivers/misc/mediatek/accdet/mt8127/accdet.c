...
/*Disable ACCDET*/
		...
		disable_accdet();
		headset_plug_out();
        ACCDET_DEBUG("Panzer [Accdet]EINT eint_accdet_sync_flag %d \n", eint_accdet_sync_flag);
	}
...
static int key_check(int b)
{
	...
	ACCDET_ERROR("[accdet] key_check value:%d \n",b);
...
static void send_key_event(int keycode, int flag)
{
    if (eint_accdet_sync_flag == 1) {
        ...
        case DW_KEY:
            ...
            ACCDET_ERROR("Panzer [accdet]KEY_VOLUMEDOWN %d\n", flag);
            break;
        case UP_KEY:
            input_report_key(kpd_accdet_dev, KEY_VOLUMEUP, flag);
            input_sync(kpd_accdet_dev);
            ACCDET_ERROR("Panzer [accdet]KEY_VOLUMEUP %d\n", flag);
            break;
        case MD_KEY:
            input_report_key(kpd_accdet_dev, KEY_PLAYPAUSE, flag);
            input_sync(kpd_accdet_dev);
            ACCDET_ERROR("Panzer [accdet_send_key_event]KEY_PLAYPAUSE %d eint_accdet_sync_flag %d\n", flag, eint_accdet_sync_flag);
            break;
        case AS_KEY:
...
{
	int ret;
    /* DTS2017041000094 wwx427562 2017-04-11 begin */
    ret = sscanf(buf, "%1s", &call_status);
    /* DTS2017041000094 wwx427562 2017-04-11 begin */
	if (ret != 1) {
...
static ssize_t store_accdet_start_debug_thread(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	int ret;
    /* DTS2017041000094 wwx427562 2017-04-11 begin */
    ret = sscanf(buf, "%1s", &start_flag);
    /* DTS2017041000094 wwx427562 2017-04-11 end */
	if (ret != 1) {
		ACCDET_DEBUG("accdet: Invalid values\n");
		return -EINVAL;
	}
