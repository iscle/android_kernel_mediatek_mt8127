...
/*< DTS2017010303755 xingbin/xwx427571 20170114 begin */
#ifdef CONFIG_LOG_JANK
#include <linux/log_jank.h>
#endif
/*< DTS2017010303755 xingbin/xwx427571 20170114 end */
...
DISP_STATUS lcd_mipi_reg_write(u32 mipi_reg,u32 reg_val)
{
	DISP_STATUS ret = DISP_STATUS_OK;

	if (down_interruptible(&sem_update_screen)) {
		pr_err("ERROR: Can't get sem_update_screen in lcd_mipi_reg_write()\n");
		return DISP_STATUS_ERROR;
	}
	...
	if (!lcm_drv->set_lcd_reg) {
		ret = DISP_STATUS_NOT_IMPLEMENTED;
		goto End;
	}
	mutex_lock(&LcmCmdMutex);
	lcm_drv->set_lcd_reg(mipi_reg,reg_val);
...
DISP_STATUS lcm_early_suspend(void)
{
	DISP_STATUS ret = DISP_STATUS_OK;
	...
	if (!lcm_drv->early_suspend) {
		ret = DISP_STATUS_NOT_IMPLEMENTED;
		goto End;
	}

	lcm_drv->early_suspend();
End:
