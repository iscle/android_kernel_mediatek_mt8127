...
/* < DTS2016061000630 guanyuwei/gwx353765 20160610 begin*/
/* define /sys/bootinfo/sec_efuse */
/* bit 0 -> sbc
** bit 1 -> sla
** bit 2 -> daa
** TBD
*/
char sec_efuse_info[][16] =
{
	"no-burn",
	"sbc",
	"sla",
...
static ssize_t sec_efuse_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int g_sec_efuse = 0;
	char *br_ptr;

	br_ptr = strstr(saved_command_line, "sec_efuse=");
	if (br_ptr != 0) {
		/* get sec_efuse */
		g_sec_efuse = br_ptr[10] - '0';
		LOGE("sec_efuse=%d\n", g_sec_efuse);
		//return sprintf(buf, "%s\n", sec_efuse_info[g_sec_efuse]);
		return sprintf(buf, "%d\n", g_sec_efuse);
	} else
		return 0;
}

static struct kobj_attribute sec_efuse_attr = __ATTR_RO(sec_efuse);

/* define /sys/bootinfo/sec_sboot */
char sec_sboot_info[][16] =
{
	"disable",
	"enable",
	"unknow"
};
static ssize_t sec_sboot_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int g_sec_sboot = 0;
	char *br_ptr;

	br_ptr = strstr(saved_command_line, "sec_sboot=");
	if (br_ptr != 0) {
		/* get sec_sboot */
		g_sec_sboot = br_ptr[10] - '0';
		LOGE("sec_sboot=%d\n", g_sec_sboot);
		//return sprintf(buf, "%s\n", sec_sboot_info[g_sec_sboot]);
		return sprintf(buf, "%d\n", g_sec_sboot);
	} else
		return 0;
}

static struct kobj_attribute sec_sboot_attr = __ATTR_RO(sec_sboot);

/* define /sys/bootinfo/sec_lock */
char sec_lock_info[][16] =
{
	"unlock",
	"lock",
	"unknow"
};
static ssize_t sec_lock_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int g_sec_lock = 0;
	char *br_ptr;

	br_ptr = strstr(saved_command_line, "sec_lock=");
	if (br_ptr != 0) {
		/* get sec_lock */
		g_sec_lock = br_ptr[9] - '0';
		LOGE("sec_lock=%d\n", g_sec_lock);
		//return sprintf(buf, "%s\n", sec_lock_info[g_sec_lock]);
		return sprintf(buf, "%d\n", g_sec_lock);
	} else
		return 0;
}

static struct kobj_attribute sec_lock_attr = __ATTR_RO(sec_lock);
/* DTS2016061000630 guanyuwei/gwx353765 20160610 end > */
/* DTS2016051300692 yangfei/ywx353749 20160718 begin */
static ssize_t wp_enable_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int y_wp_enable = 0;
	char *br_ptr;

	br_ptr = strstr(saved_command_line, "wp_enable=");
	if (br_ptr != 0) {
		y_wp_enable = br_ptr[10] - '0';
		LOGE("wp_enable=%d\n", y_wp_enable);
		return sprintf(buf, "%d\n", y_wp_enable);
	} else
		return 0;
}
static struct kobj_attribute wp_enable_attr = __ATTR_RO(wp_enable);
/* DTS2016051300692 yangfei/ywx353749 20160718 end */
