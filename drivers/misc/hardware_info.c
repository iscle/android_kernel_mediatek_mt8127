...
#define HARDWARE_INFO_ATTR(name)\
	...
	HARDWARE_INFO_ATTR(sar);
	HARDWARE_INFO_ATTR(hall);
...
static int hardware_info_probe(struct platform_device *pdev)
{
	...
	/*add sensor hardware info*/
	register_hardware_info(G_SENSOR, "sorry,gsensor not detected");
	register_hardware_info(M_SENSOR, "sorry,msensor not detected");
	register_hardware_info(ALS_PS, "sorry,als and ps not detected");
	register_hardware_info(GYRO, "sorry,gyro not detected");
	register_hardware_info(SAR, "sorry,sar not detected");
	register_hardware_info(HALL, "sorry,hall not detected");
	/*add sensor hardware info*/
...
int register_hardware_info(HARDWARE_ID id, char* name)
{
	...
	case GYRO:
		...
		break;
	case SAR:
		snprintf(hardware_info->sar, 125,name);
		ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_sar);
		if (ret) {
			printk("%s:error on device_create_file %s\n", __func__, name);
		}
		break;
	case HALL:
		snprintf(hardware_info->hall, 125,name);
		ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_hall);
		if (ret) {
			printk("%s:error on device_create_file %s\n", __func__, name);
		}
		break;
	...
	#ifdef CONFIG_MTK_MULTI_BAT_PROFILE_SUPPORT
	case BATTERY_TYPE:
			snprintf(hardware_info->battery_type, 125,name);
			ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_battery_type);
			if (ret) {
			  printk("%s:error on device_create_file %s\n", __func__, name);
			}
		break;
...
/*DTS2016122706947 ywx422261 20161227 begin */
	case GPS:
		snprintf(hardware_info->gps, HW_INFO_MAX_LEN,name);
		ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_gps);
		if (ret) {
			printk("%s:error on device_create_file %s\n", __func__, name);
		}
		break;
...
/*DTS2016122500642 ywx422261 20161227 begin */
	case HARDWARE_VER:
		snprintf(hardware_info->hardware_ver, HW_INFO_MAX_LEN,name);
		ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_hardware_ver);
		if (ret) {
			printk("%s:error on device_create_file %s\n", __func__, name);
		}
		break;
