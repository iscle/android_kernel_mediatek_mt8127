...
#define GT9XX_FW_NAME      "gt9xx/gt9xx_fw.bin"
#endif
...
u8 searching_file = 0;
u8 got_file_flag = 0;
u8 gtp_loading_fw = false;
u8 gtp_force_loading_fw = 0;
...
static int gt9xx_upgrade_open(struct inode *inode, struct file *file)
{
	return 0;
}
static int gt9xx_upgrade_close(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t gt9xx_upgrade_write(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
	char buffer[PROC_UPGRADE_NUMBUF]={0};
	char min_len=0;
	s32 ret = 0;
	min_len=(count<PROC_UPGRADE_NUMBUF?count:PROC_UPGRADE_NUMBUF);
	memset(buffer, 0, sizeof(buffer));
	if (copy_from_user(buffer, buf, min_len))
		return -EFAULT;
	if(strncmp( buffer, "force_upgrade", PROC_UPGRADE_NUMBUF )){
		GTP_ERROR("force upgrade failed, the input str must be force_upgrade ");
		return -EFAULT;
	}
	if(gtp_force_loading_fw==FORCE_LOADING_STATUS_OPENING){
		GTP_ERROR("force upgrade failed, force_upgrade should not run twice!!");
		return -EFAULT;
	}
	gtp_force_loading_fw=FORCE_LOADING_STATUS_OPENING;
	/*please make sure the power is on when force upgrade the firmware*/
	ret = gup_update_proc("/sdcard/gt9xx_fw.bin");
	if(ret == FAIL)
	{
		GTP_ERROR("[gt9xx_upgrade_write]force upgrade failed, Please make sure the power is on!");
	}
	gtp_force_loading_fw=FORCE_LOADING_STATUS_END;

	return count;
}


static const struct file_operations gt9xx_upgrade_fops = {
	.write = gt9xx_upgrade_write,
	.open = gt9xx_upgrade_open,
	.release = gt9xx_upgrade_close,
};


static struct proc_dir_entry *tp_root=NULL;

s32 init_manual_upgrade_node(void)
{
	struct proc_dir_entry *pentry_upgrade=NULL;

	tp_root = proc_mkdir(NAME_TOUCHSCREEN_PROC_DIR, NULL);
	if (!tp_root)
	{
		GTP_ERROR("unable to create /proc/%s entry\n", NAME_TOUCHSCREEN_PROC_DIR);
		return -FAIL;
	}

	pentry_upgrade = proc_create(NAME_MANUAL_UPGRADE, 0, tp_root, &gt9xx_upgrade_fops);
	if (pentry_upgrade == NULL)
	{
		GTP_ERROR("unable to create /proc/%s/%s entry\n", NAME_TOUCHSCREEN_PROC_DIR,NAME_MANUAL_UPGRADE);
 		return -FAIL;
 	}
...
void uninit_manual_upgrade_node(void){
	remove_proc_entry(NAME_MANUAL_UPGRADE, tp_root);
}
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8 *msg, s32 len)
{
	...
	if (i >= I2C_BYTES_RETRY_TIMES_MAX) {
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	for (retry = 0; retry < 2; retry++) {
		...
		if (FAIL == ret) {
			...
			gt9293_dsm_report_err(DSM_TP_FW_CRC_ERROR_NO,GT9293_DSM_TYPE_UPDATE_FW_ERR);
			#endif
...
static u8 gup_enter_update_judge(struct st_fw_head *fw_head)
{
	...
	(!memcmp(fw_head->pid, "91XX", 4))) {
		...
		if(gtp_force_loading_fw==FORCE_LOADING_STATUS_OPENING){
			GTP_ERROR("Force upgrade,bypass judge vid!");
			return SUCCESS;
		}
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	if (buf == NULL) {
		GTP_ERROR("[gup_update_config]Alloc memory for buf failed");
		goto update_cfg_file_failed;
	}
	pre_buf = kzalloc(file_len, GFP_KERNEL);
	data = (u8 *) kzalloc(data_length, GFP_KERNEL);
	if (pre_buf == NULL) {
		GTP_ERROR("[gup_update_config]Alloc memory for pre_buf failed");
		goto update_cfg_file_failed;
	}
	file_config = kzalloc(chip_cfg_len + GTP_ADDR_LENGTH, GFP_KERNEL);
	data = (u8 *) kzalloc(data_length, GFP_KERNEL);
	if (file_config == NULL) {
		GTP_ERROR("[gup_update_config]Alloc memory for file_config failed");
		goto update_cfg_file_failed;
	}
	...
	i = 0;
	while (i++ < I2C_BYTES_RETRY_TIMES_MAX) {
...
static u8 gup_check_update_file(struct i2c_client *client, struct st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		...
		if (ret < 0) {
			/*DTS2016122100332,yuquan/ywx422261 added DSM for touchpanel,begin @20161214 */
			#ifdef CONFIG_HUAWEI_DSM
			gt9293_dsm_report_err(DSM_TP_FWUPDATE_ERROR_NO,GT9293_DSM_TYPE_PARSE_FW_ERR);
			#endif
			/*DTS2016122100332,yuquan/ywx422261 added DSM for touchpanel,end @20161214 */
		...
		{
			#ifdef CONFIG_HUAWEI_DSM
			gt9293_dsm_report_err(DSM_TP_I2C_RW_ERROR_NO,GT9293_DSM_TYPE_CHECK_SUM_ERR);
			#endif
			return FAIL;
		}
		/*DTS2016122100332,yuquan/ywx422261 added DSM for touchpanel,end @20161214 */
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	if (got_file_flag == HEADER_FW_READY)
	{
		if(SEEK_SET == set_or_end)
			memcpy(buf, &update_msg.fw->data[FW_HEAD_LENGTH + offset], length);
		else    /*seek end*/
			memcpy(buf, &update_msg.fw->data[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
		return SUCCESS;
	}
#endif
	{
...
s32 gup_update_proc(void *dir)
{
	...
	wake_lock(&tp_gt9293_suspend_lock);
	...
	update_msg.fw = NULL;
	memset(&fw_head, 0x00, sizeof(fw_head));
	...
	if (FAIL == ret) {
		/*DTS2016122100332,yuquan/ywx422261 added DSM for touchpanel,begin @20161214 */
		#ifdef CONFIG_HUAWEI_DSM
		gt9293_dsm_report_err(DSM_TP_FWUPDATE_ERROR_NO,GT9293_DSM_TYPE_UPDATE_FW_ERR);
		#endif
		/*DTS2016122100332,yuquan/ywx422261 added DSM for touchpanel,end @20161214 */
...
#endif
	/*DTS2017012205954 shangqiang/ywx422251 20170122 begin */
	wake_unlock(&tp_gt9293_suspend_lock);
	/*DTS2017012205954 shangqiang/ywx422251 20170122 end */
	total_len = 100;
	if (SUCCESS == update_ret) {
		show_len = 100;
...
s32 gup_load_by_bank(u8 bank, u8 need_check, const u8 *fw, u32 length)
{
	...
	while (retry++ < I2C_BYTES_RETRY_TIMES_MAX) {
		...
		if (need_check) {
			ret = gup_check_and_repair(i2c_client_point, 0xC000, (u8 *)fw, length);
			if (FAIL == ret) {
...
s32 gup_load_authorization_fw(void)
{
	...
	GTP_INFO("Request firmware...");
	ret = request_firmware(&update_msg.fw, GT9XX_HN_AUTH_FW_NAME, &i2c_client_point->dev);
	if (ret < 0) {
		GTP_ERROR("Request firmware failed - %s (%d)\n",
