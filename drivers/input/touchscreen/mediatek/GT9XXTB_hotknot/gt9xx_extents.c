...
s8 gtp_enter_doze(void)
{
	...
	while(retry++ < I2C_BYTES_RETRY_TIMES_MAX) {
		ret = ges_i2c_write_bytes(GTP_REG_ENHANCE_ESD, i2c_control_buf, 1);
		if (ret < 0) {
			GTP_DEBUG("failed to set doze flag into 0x8046, %d", retry);
			continue;
		}

		ret = ges_i2c_write_bytes(GTP_REG_SLEEP, i2c_control_buf, 1);
		if (!ret){
...
static s32 io_iic_write(u8 * data)
{
	...
	if(sizeof(data) <= 4)
	{
		GTP_ERROR("[io_iic_write]data array is error");
		return -1;
	}
	addr = data[0] << 8 | data[1];
	data_length = data[2] << 8 | data[3];
...
static long gtp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd)) {
		...
		if (data == NULL) {
			GTP_ERROR("[gtp_ioctl]Alloc memory for data failed");
			return -1;
		}
