	...
	if (sock_tag_entry != SEQ_START_TOKEN) {
		...
		f_count = atomic_long_read(
			&sock_tag_entry->socket->file->f_count);
                /** DTS2016111505180 chenxing/wwx329951 20161115 begin */
	        seq_printf(m, "sock=%pK tag=0x%llx (uid=%u) pid=%u "
                           "f_count=%lu\n",
			   sock_tag_entry->sk,
			   sock_tag_entry->tag, uid,
			   sock_tag_entry->pid, f_count);
               /** DTS2016111505180 chenxing/wwx329951 20161115 end */
	} else {
			 ...
			 int cnt_set)
{
	...
	/* Detailed tags are not available to everybody */
      /** <DTS2016091203944 liangshizhou/lwx367803 20160912 begin */
	if (!can_read_other_uid_stats(
						make_kuid(&init_user_ns,stat_uid))) {
      /** DTS2016091203944 liangshizhou/lwx367803 20160912 end> */
